...
class ProformaUygulamasi:
    ...
    def arayuz_olustur(self):
        ...
        self.btn_tema = ttk.Button(ust_bar, text="🌙 Koyu Mod", command=self.tema_degistir)
        self.btn_tema.pack(side=tk.RIGHT, padx=5)

        # --- Ana Panel Bölümü ---
        ana_panel = ttk.Frame(self.root)
        ...
        panel_sag = ttk.Frame(ana_panel, width=350)
        ...
        # Müşteri Seçimi
        ttk.Label(frame_musteri, text="Kayıtlı Müşteri Seç:", anchor="w").pack(fill=tk.X)
        ...
        ttk.Label(frame_sepet, text="Para Birimi", anchor="w").pack(fill=tk.X)
        self.var_para_birimi = tk.StringVar(value="USD")
        cerceve_para = ttk.Frame(frame_sepet)
        ...
        ttk.Button(frame_sepet, text="➖ Seçili Ürünü Sepetten Çıkar", command=self.secili_urunu_cikar).pack(fill=tk.X, pady=2)
        ttk.Button(frame_sepet, text="🗑️ Sepeti Boşalt", command=self.sepeti_bosalt).pack(fill=tk.X, pady=2)

    def tema_degistir(self):
        self.koyu_mod = getattr(self, "koyu_mod", False)
        self.koyu_mod = not self.koyu_mod
        style = ttk.Style()
        
        bg_dark = "#2b2b2b"
        fg_dark = "#ffffff"
        bg_light = "SystemButtonFace"
        fg_light = "#000000"
        ...
        if self.koyu_mod:
            ...
            style.configure("TRadiobutton", background=bg_dark, foreground=fg_dark)
            
            style.configure("Treeview", background="#333333", foreground=fg_dark, fieldbackground="#333333")
            style.configure("Treeview.Heading", background="#404040", foreground=fg_dark)
            
            self.listbox_sepet.config(bg="#333333", fg=fg_dark, selectbackground="#555555")
            self.btn_tema.config(text="☀️ Açık Mod")
        else:
            ...
            style.configure("TRadiobutton", background=bg_light, foreground=fg_light)
            
            style.configure("Treeview", background="white", foreground=fg_light, fieldbackground="white")
            style.configure("Treeview.Heading", background="#e1e1e1", foreground=fg_light)
            
            self.listbox_sepet.config(bg="white", fg=fg_light, selectbackground="#0078D7")
            self.btn_tema.config(text="🌙 Koyu Mod")

    def input_yap(self, parent, label_text):
        ttk.Label(parent, text=label_text, anchor="w").pack(fill=tk.X)
    ...
    def secili_urunu_cikar(self):
        secili_indeksler = self.listbox_sepet.curselection()
        if not secili_indeksler:
            messagebox.showwarning("Uyarı", "Lütfen sepetten çıkarmak için bir ürün seçin.")
            return
            
        index = int(secili_indeksler[0])
        urun = self.sepet[index]
        
        # Stok iadesi (DB'ye)
        self.db.urun_stok_guncelle(urun["id"], 1)
    ...
    def yonetici_penceresi_ac(self):
        ...
        for lbl, key in fields:
            ttk.Label(frame_form, text=lbl, anchor="w").pack(fill=tk.X)
            ent = ttk.Entry(frame_form)
            ent.pack(fill=tk.X, pady=(0, 5))
            entries[key] = ent

        aktif_id_var = tk.StringVar(value="")
        lbl_aktif = tk.Label(frame_form, text="Yeni Ürün Eklenecek", fg="blue", font=("", 10, "italic"), bg=self.root["bg"])
        ...
        def form_temizle():
            for ent in entries.values():
                ent.delete(0, tk.END)
            aktif_id_var.set("")
            lbl_aktif.config(text="Yeni Ürün Eklenecek", fg="#3498db" if getattr(self, "koyu_mod", False) else "blue", bg=self.root["bg"])
        ...
        def form_doldur(event):
            ...
            lbl_aktif.config(text=f"ID: {urun_id} Düzenleniyor", fg="#e74c3c" if getattr(self, "koyu_mod", False) else "red", bg=self.root["bg"])
        ...
        btn_cerceve = ttk.Frame(frame_form)
    ...
    def excel_olustur(self):
        ...
        try:
            ...
            # --- ÜRÜNLERİ YAZDIRMA ---
            # Ürün yazdırma işlemi kesinlikle tam 18. satırdan başlayacak.
            # 17. satır tamamen boş bırakılacak, hiçbir veri yazılmayacak.
            baslangic_satiri = 18
            ...
            for bos_satir in range(son_dolu_satir, 30):
                ws.cell(row=bos_satir, column=2).value = ""
                ws.cell(row=bos_satir, column=3).value = ""
                ws.cell(row=bos_satir, column=4).value = ""
                ws.cell(row=bos_satir, column=5).value = ""
